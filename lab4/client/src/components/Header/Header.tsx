...
const Header = () => {
  ...
  return (
    <header>
      ...
      <div>
        <img src={Glass} />
        <input onChange={handleInput} type="text" placeholder="Search by author, title, name" />
      </div>
      {!tokenIsAccurate && (
        <div>
          <a
            className={isCurrentPath('/log_in') ? 'current_page' : ''}
            onClick={() => navigate('/log_in')}
          >
            Log in
          </a>
          <a
            className={isCurrentPath('/sign_up') ? 'current_page' : ''}
      ...
      {tokenIsAccurate && (
        <div>
          <a className={isCurrentPath('/') ? 'current_page' : ''} onClick={() => navigate('/')}>
            All books
          </a>
          <a
            className={isCurrentPath('/orders') ? 'current_page' : ''}
